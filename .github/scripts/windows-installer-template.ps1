...
function Test-JavaFxAvailable {
    try {
        $mods = & java --list-modules 2>$null | Select-String "javafx.controls" -ErrorAction SilentlyContinue
        if ($mods) { return $true }
    } catch { }
    return $false
}
...
if not exist "%JAVAFX_CACHE%" (
    mkdir "%JAVAFX_CACHE%" >nul 2>&1
)

"%JAVA_CMD%" --add-modules=javafx.controls -Djavafx.cachedir="%JAVAFX_CACHE%" -jar "%APP_DIR%woodlanders-launcher.jar"
if %ERRORLEVEL% neq 0 (
    echo.
...
try {
    ...
    # Check if Java is installed and has JavaFX; otherwise download bundled JRE
    $needBundledJre = $true
    if (Test-JavaInstalled) {
        if (Test-JavaFxAvailable) {
            Write-Success "Java 17+ with JavaFX found on system"
            $needBundledJre = $false
        } else {
            Write-Status "System Java lacks JavaFX; will install bundled JDK with JavaFX"
        }
    }
    if ($needBundledJre -and -not (Download-JRE)) {
